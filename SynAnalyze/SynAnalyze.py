...
class SynAnalyze(object):
    ...
    def getClosure(self, cur_item, item_set):
        ...
        tail_set = cur_item[4]
        # 圆点后为非终结符才继续增大项目集
        if point_index < len(right_list) and (right_list[point_index]
                                              in self.nonTerminators):
        ...
        while queue:
            ...
            for item in now_item_set:
                ...
                if point_index >= len(right_list) or "$" in right_list:  # 归约
                    if now_node_id not in self.LRTable.keys(
                    ):  # 为当前项目集在LR分析表中创立表项
                        self.LRTable[now_node_id] = dict()
                    for tail in tail_set:
                        if tail in self.LRTable[now_node_id].keys():
                            # 此处由于产生式中的if else部分具有二义性故会出错，但是人为认定else跟随较近的if之后屏蔽报错
                            pass
                    ...
                    next_item = (pro_id, left, right_list, point_index + 1,
                                 tail_set)
                    ...
                    for ex_item in now_item_set:  # 对后一个字符相同的其他产生式也要求闭包，并取并
                        ...
                        if (ex_point_index < len(ex_right_list)
                                and ex_right_list[ex_point_index] == next_c):
                    ...
                    if tuple(
                            next_item_set) in all_item_set.keys():  # 该项目集已经处理过
                    ...
                    if now_node_id not in self.LRTable.keys(
                        ...
                        self.LRTable[now_node_id] = dict()
                    if right_list[point_index] in self.LRTable[
                            now_node_id].keys():
                        print("当前文法不属于LR(1)文法！！！")
                        return False
        ...
        while True:
            ...
            if now_token in self.LRTable[top_status].keys():  # 进行状态转移
                ...
                if action[0] == "acc":
                    ...
                    symbol_stack.append(
                        (now_token_name, now_token, 0, tree_layer_num[0]))
                    tree_layer.append((now_token, 0, tree_layer_num[0]))
                    tokens = tokens[:-1]
                elif action[0] == "r":
                    production = self.productions[action[1]]
                    token_list = list()
                    left = list(production.keys())[0]  # 待规约的产生式左部
                    next_line = 0
                    if production[left] != ["$"]:  # 产生式右部不为空，不需修改两个栈
                        right_length = len(production[left])
                        status_stack = status_stack[:
                                                    -right_length]  # 将产生式右部长度大小的状态栈去除
                        ...
                        for i in range(
                            ...
                            next_line = max(next_line, symbol_stack[i][2])
                            tree_line.append(
                                [symbol_stack[i][2], symbol_stack[i][3], 0, 0])
                            token_list.insert(0, symbol_stack.pop(i))
                        next_line += 1
                    else:
                        next_line = 1
                        right_length = 1
                    ...
                    SemAnalyze_object.SemAct(action[1], token_list,
                                             now_line_num)
                    ...
                    status_stack.append(go[1])  # 查goto表
                    symbol_stack.append(
    ...
    def get_tree(self, tree_layer, tree_line, tree_path):
        ...
        Syn_tree = Tree().add("", data, orient="TB",
                              is_roam=True).set_global_opts(
    ...
    def analyze(self, token_table_path, tree_path, SynAnalyzeProcess_path,
        ...
        for line in token_table:
            line = line[:-1]  # 去\n
            next_token = line.split(" ")
            if next_token[1] in ["identifier", "number"]:
                tokens.append((next_token[0], next_token[2], next_token[1]))
            else:
                tokens.append(tuple(next_token))
        tokens.append((str(0), "start", "#"))
        token_table.close()
        isSuccess, tree_layer, tree_line, message = self.runOnLRTable(
            tokens, SynAnalyzeProcess_path, Sem_object)  # 分析
        if isSuccess:  # 成功
            self.get_tree(tree_layer, tree_line, tree_path)
            print("Syn analyze complete!")
            return True, message
        else:
            print("Syn analyze failed!")
...
if __name__ == "__main__":
    ...
    tree_path = "./templates/render.html"  # 树地址
    SynAnalyzeProcess_path = "./SynAnalyze/runOnLRTable/runOnLRTable.txt"  # 分析栈地址
    Sem_ana = SemAnalyze()
    syn_ana = SynAnalyze()
    ...
    syn_ana.analyze(TokenTable_path, tree_path, SynAnalyzeProcess_path,
                    Sem_ana)
