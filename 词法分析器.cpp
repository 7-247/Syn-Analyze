...
int searchReserve(string s) {
    ...
    return (iter != word.end())
               ? iter->second
               : -1;  //若成功查找，则返回种别码；否则返回100，代表查找不成功，即为标识符
}
...
void Scanner(string resProgram, int &pointer) {
    ...
    while (ch == ' ')
        ch = resProgram[++pointer];  //过滤空格，防止程序因识别不了空格而结束
    if (isalpha(resProgram[pointer]) || isdigit(resProgram[pointer])) {
        if (isalpha(resProgram[pointer])) {  //开头为字母，后跟字母或数字
            while (isalpha(resProgram[pointer]) || isdigit(resProgram[pointer]))
                token += resProgram[pointer++];  //收集，然后下移
            syn = searchReserve(
                token);  // 查表，若是保留字或已出现过的终结符，返回种别码；
            if (syn == -1) {
                iden << token << endl;
                token = "identifier";
            }
            ...
            while (isdigit(resProgram[pointer]))
                token += resProgram[pointer++];  //开头为数字，后跟数字。
